...
const ApprovisionnementModal = ({ isOpen, onClose, product }) => {
  ...
  const { data: allBoutiques = [], isLoading: loadingBoutiques } = useBoutiques();
  ...
  const handleSubmit = async (e) => {
    ...
    if (mode === "direct") {
      // useApprovisionner attend { productId, data: { quantity, unit } }
            ...
            <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto">
              <div className="px-6 py-5 space-y-5">
                ...
                <div className="grid grid-cols-2 gap-2">
                  {[
                    ...
                    <button key={key} type="button" onClick={() => setMode(key)}
                      className={`p-3.5 rounded-2xl border-2 text-left transition-all ${mode === key ? "border-slate-900 bg-slate-900" : "border-slate-200 bg-white hover:border-slate-300"}`}>
                ...
                <div className="bg-slate-50 rounded-2xl p-4 grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <div className="flex items-center gap-1.5 mt-1">
                      ...
                      <span className="text-sm font-semibold text-slate-800">{product.boutique_id?.name || "—"}</span>
                    </div>
                  </div>
                  <div>
                    <p className="text-[10px] uppercase tracking-wide font-bold text-slate-400">Stock actuel</p>
                    <p className="text-sm font-black text-slate-800 mt-1">{stockActuel} <span className="font-medium text-slate-500">{product.unit}</span></p>
                  </div>
                  <div>
                    <p className="text-[10px] uppercase tracking-wide font-bold text-slate-400">Prix unit.</p>
                    <p className="text-sm font-black text-emerald-700 mt-1">{(product.basePrice || 0).toLocaleString("fr-FR")} FCFA</p>
                  </div>
                  <div>
                    <p className="text-[10px] uppercase tracking-wide font-bold text-slate-400">Valeur stock</p>
                    <p className="text-sm font-black text-purple-700 mt-1">{((product.basePrice || 0) * stockActuel).toLocaleString("fr-FR")} F</p>
                  </div>
                </div>
                    ...
                    {loadingBoutiques ? (
                      ...
                      <div className="relative">
                        ...
                        {autresBoutiques.length === 0 && (
                          <p className="text-xs text-amber-600 mt-1">Aucune autre boutique disponible</p>
                        )}
                      </div>
                    )}
                  </Field>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <Field label="Quantité" required>
                    <input type="number" name="quantity" value={form.quantity} onChange={handleChange}
                      min="0.01" step="0.01" placeholder="Ex: 100" className={inputCls} required disabled={isLoading} />
                  </Field>
                  <Field label="Unité" hint={form.unit !== product.unit ? `Sera converti en ${product.unit}` : "Unité de base"}>
                    <div className="relative">
                ...
                {qty > 0 && (
                  <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
                    ...
                    <p className="text-xs font-black uppercase tracking-widest text-emerald-600 mb-3">Aperçu après opération</p>
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        ...
                        <p className="text-2xl font-black text-emerald-800 mt-0.5">
                          {nouveauStock}<span className="text-sm font-medium text-emerald-600 ml-1">{product.unit}</span>
                        </p>
                      ...
                      <div>
                        ...
                        <p className="text-2xl font-black text-emerald-800 mt-0.5">
                          {nouvelleValeur.toLocaleString("fr-FR")}<span className="text-xs font-medium text-emerald-600 ml-1">F</span>
                        </p>
              ...
              <div className="sticky bottom-0 bg-white border-t border-slate-100 px-6 py-4 flex gap-3">
                ...
                <button type="submit" disabled={!canSubmit}
                  className={`flex-1 py-3 rounded-xl text-white font-semibold text-sm disabled:opacity-40 disabled:cursor-not-allowed flex items-center justify-center gap-2 ${mode === "direct" ? "bg-emerald-600 hover:bg-emerald-700" : "bg-blue-600 hover:bg-blue-700"}`}>
                  {isLoading ? (
