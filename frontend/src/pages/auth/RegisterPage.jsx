...
import { Mail, Lock, Eye, EyeOff, Store, User, ArrowRight } from "lucide-react";
import { useRegister } from "../../hooks/useAuth";

const RegisterPage = ({ onSwitchToLogin }) => {
    const [form, setForm] = useState({ email: "", password: "", name: "", boutiqueName: "" });
    ...
    const handleSubmit = (e) => {
        ...
        setError("");
        register(
            { email: form.email, password: form.password, name: form.name, boutiqueName: form.boutiqueName },
            {
                onError: (err) => setError(err?.response?.data?.error || "Erreur lors de l'inscription"),
                onSuccess: (data) => { if (data?.error) setError(data.error); },
            }
        );
    };

    const inputCls = "w-full py-3 bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-slate-900/20 focus:border-slate-400 text-slate-700 placeholder-slate-400 text-sm transition-all disabled:opacity-50";

    const fields = [
        { name: "boutiqueName", label: "Nom de votre boutique", placeholder: "Ex: Boutique Centrale", icon: Store, type: "text" },
        { name: "name", label: "Votre nom", placeholder: "Ex: Jean Dupont", icon: User, type: "text" },
        { name: "email", label: "Email", placeholder: "vous@exemple.com", icon: Mail, type: "email" },
    ];
            ...
            >
                <div className="bg-white/80 backdrop-blur-xl rounded-3xl shadow-2xl border border-white/20 p-8 sm:p-10">
                    <div className="text-center mb-8">
                        ...
                        <p className="text-sm text-slate-500 mt-1">14 jours d'essai gratuit</p>
                    </div>
                        ...
                        <div className="space-y-1">
                            ...
                            <div className="relative">
                                ...
                                <input type={showPassword ? "text" : "password"} name="password"
                                    ...
                                    onChange={handleChange} required disabled={isPending}
