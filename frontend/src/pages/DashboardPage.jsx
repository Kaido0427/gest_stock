...
const DashboardPage = () => {
    ...
    const { data: statsJour, isLoading: loadingStatsJ, refetch: refetchStatsJ } = useStatistiquesVentes({ periode: "jour", boutique_id: boutiqueId });
    const { data: statsMois, isLoading: loadingStatsM, refetch: refetchStatsM } = useStatistiquesVentes({ periode: "mois", boutique_id: boutiqueId });
    const { data: historiqueData, isLoading: loadingH, refetch: refetchH } = useHistoriqueVentes({ limit: 50, boutique_id: boutiqueId });
    const { data: alertesData, isLoading: loadingA, refetch: refetchA } = useAlertesStock({ seuil: 10, boutique_id: boutiqueId });
    ...
    const stats = {
        today: {
            sales: statsJour?.data?.global?.montantTotal ?? 0,
            transactions: statsJour?.data?.global?.totalVentes ?? 0,
            avgBasket: statsJour?.data?.global?.moyennePanier ?? 0,
        },
        month: {
            sales: statsMois?.data?.global?.montantTotal ?? 0,
            transactions: statsMois?.data?.global?.totalVentes ?? 0,
        },
    };
            ...
            <div className="max-w-7xl mx-auto space-y-5">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
                    {loadingData ? Array(4).fill(0).map((_, i) => <Skeleton key={i} className="h-28" />) : (
                        <>
                            <KpiCard label="Ventes du jour" value={`${fmt(stats.today.sales)} FCFA`} sub={`${stats.today.transactions} transaction(s)`} icon={TrendingUp} accent="emerald" delay={0.05} />
                            <KpiCard label="Ventes du mois" value={`${fmt(stats.month.sales)} FCFA`} sub={`${stats.month.transactions} transaction(s)`} icon={ShoppingCart} accent="blue" delay={0.1} />
                            <KpiCard label="Panier moyen" value={`${fmt(stats.today.avgBasket)} FCFA`} icon={BarChart2} accent="amber" delay={0.15} />
                            <KpiCard label="Alertes stock" value={alertes.length} sub="produits à réappro." icon={AlertTriangle} accent={alertes.length > 0 ? "red" : "emerald"} delay={0.2} />
                        </>
                    )}
                </div>
                    ...
                    {chartData.length === 0 ? (
                        ...
                        <ResponsiveContainer width="100%" height={200}>
                            <AreaChart data={chartData} margin={{ top: 4, right: 4, left: 0, bottom: 0 }}>
                                ...
                                <Area type="monotone" dataKey="montant" name="Montant" stroke="#6366f1" strokeWidth={2.5} fill="url(#colorMontant)" dot={false} activeDot={{ r: 5, fill: "#6366f1" }} />
                            </AreaChart>
                        </ResponsiveContainer>
                    )}
                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                    <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5">
                        ...
                        {loadingData ? <div className="space-y-3">{Array(5).fill(0).map((_, i) => <Skeleton key={i} className="h-8" />)}</div> : <TopProduits data={topProduits} />}
                    </div>
